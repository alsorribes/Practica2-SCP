...
public class Ocean
{
	...
	public Point[] getFreeNeighbours(int x, int y)
	{
		...
		for (int i = 0; i < NEIGHBOUR_MASK.length; i++) {
			...
			if ( neighbour_pos.x<0 || neighbour_pos.x>=getWidth() || neighbour_pos.y<0 || neighbour_pos.y>=getHeight() )
				System.err.println("Error coordenadas");
		}
	...
	public static class Shark extends Cell {
		...
		// Implements shark simulation rules
		@Override
		protected void internalUpdate(Ocean o, int x, int y, int generation, 
				int fishCycle, int sharkCycle) {
			Point[] fishNeighbors;
			...
			for (Point fish : fishNeighbors)
				if (o.getField(fish.x,fish.y).getColor()==Cell.FISH) {
